            ...
            foreach ($this->id_array as $key => $id) {
                ...
                if (isset($this->slides[$id]) || $slide_type == 'video') {
                    ...
                    if (trim($title) != '') {
                        ...
                        if ($counter === 2) {
                            $title = "<{$heading} {$title_styling} class='avia-caption-title gloria-avia-caption-title-white {$css} {$av_title_font_classes}' $markup_name>" . trim(apply_filters('avf_slideshow_title', $title)) . "</{$heading}>";

                        }
                        elseif ($counter === 3) {
                            $title = "<{$heading} {$title_styling} class='avia-caption-title gloria-avia-caption-title-green {$css} {$av_title_font_classes}' $markup_name>" . trim(apply_filters('avf_slideshow_title', $title)) . "</{$heading}>";
                        }
                        else {
                            $title = "<{$heading} {$title_styling} class='avia-caption-title gloria-avia-caption-title-gray {$css} {$av_title_font_classes}' $markup_name>" . trim(apply_filters('avf_slideshow_title', $title)) . "</{$heading}>";
                        }
                    ...
                    if ($counter === 2) {
                        if (trim($content) != "") $content = "<div class='avia-caption-content gloria-avia-caption-content-white {$av_font_classes} {$content_class}' {$markup_description} {$content_styling}>" . ShortcodeHelper::avia_apply_autop(ShortcodeHelper::avia_remove_autop(trim($content))) . "</div>";
                    }
                    elseif ($counter === 3) {
                        if (trim($content) != "") $content = "<div class='avia-caption-content gloria-avia-caption-content-green {$av_font_classes} {$content_class}' {$markup_description} {$content_styling}>" . ShortcodeHelper::avia_apply_autop(ShortcodeHelper::avia_remove_autop(trim($content))) . "</div>";
                    }
                    elseif ($counter === 4) {
                        if (trim($content) != "") $content = "<div class='avia-caption-content gloria-avia-caption-content-green {$av_font_classes} {$content_class}' {$markup_description} {$content_styling}>" . ShortcodeHelper::avia_apply_autop(ShortcodeHelper::avia_remove_autop(trim($content))) . "</div>";
                    }
                    else {
                        if (trim($content) != "") $content = "<div class='avia-caption-content gloria-avia-caption-content-gray {$av_font_classes} {$content_class}' {$markup_description} {$content_styling}>" . ShortcodeHelper::avia_apply_autop(ShortcodeHelper::avia_remove_autop(trim($content))) . "</div>";
                    }
...
if (!class_exists('avia_slideshow_video_helper')) {
    class avia_slideshow_video_helper
    {
        ...
        static function set_video_slide($video_url, $service = false, $meta = false, $config = false)
        {
            ...
            switch ($service) {
                case "html5":
                    $video = "<div class='av-click-overlay'></div>" . avia_html5_video_embed($video_url, $fallback_img, $types = array('webm' => 'type="video/webm"', 'mp4' => 'type="video/mp4"', 'ogv' => 'type="video/ogg"'), $atts);
                    break;
                case "iframe":
                    $video = $video_url;
                    break;
